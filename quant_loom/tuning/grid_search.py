...
class GridSearchTuner:
    ...
    def generate_combinations(self, alert_type: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"{alert_type}: {len(combinations)} 种参数组合")
        return combinations
    ...
    def export_best_config(self, alert_type: str, best_params: Dict[str, Any],
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            f.write("# QuantLoom 调优后规则配置 (auto-generated by GridSearchTuner)\n")
            f.write(f"# 最优异动类型: {alert_type}\n")
            f.write(f"# 最优参数: {json.dumps(best_params)}\n\n")
            yaml.dump(config, f, allow_unicode=True, default_flow_style=False)

        logger.info(f"最优配置已导出: {output_path} (type={alert_type})")
        return output_path
